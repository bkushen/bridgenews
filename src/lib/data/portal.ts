...
const DISTRICT_ALIASES: Record<DistrictName, string[]> = {
  ...
  Kilinochchi:["kilinochchi","කිලිනොච්චි","கிளிநொச்சி"],
...
async function getRegionalArticleIds(region: EditionRegion, limit = 5000): Promise<string[]> {
  ...
  const links = await supabase.from("article_regions").select("article_id").eq("region_id", regionRow.data.id).limit(limit);
  if (links.error) throw links.error;
  return (links.data ?? []).map((row) => row.article_id);
}
...
async function hydrateArticles(rows:any[], viewCounts=new Map<string,number>()):Promise<PortalStory[]>{
  ...
  const categoryIds=[...new Set((categoryLinks.data??[]).map(r=>r.category_id))];
  ...
  for(const link of categoryLinks.data??[]){const name=categoryNames.get(link.category_id);if(name&&!categoryByArticle.has(link.article_id))categoryByArticle.set(link.article_id,name);}
...
export async function getPortalStories(options:{date?:string;language?:string;sourceSlug?:string;region?:EditionRegion;limit?:number}={}):Promise<PortalStory[]>{
  try{
    ...
    if(options.sourceSlug){const source=await supabase.from("sources").select("id").eq("slug",options.sourceSlug).maybeSingle();if(source.error||!source.data)return[];sourceId=source.data.id;}
    const regionalIds=options.region?await getRegionalArticleIds(options.region):null;
    if(options.region&&!regionalIds?.length)return[];
    ...
    if(regionalIds)query=query.in("id",regionalIds);
...
export async function getSourceDirectory(limit=100, region?:EditionRegion):Promise<SourceDirectoryItem[]>{
  try{
    const supabase=await createClient();
    const{data:sources,error}=await supabase.from("sources").select("id,name,slug,website_url,logo_url,default_language_code,last_success_at").eq("enabled",true).order("name",{ascending:true}).limit(limit);
    if(error)throw error;
    if(!sources?.length)return[];
    const ids=sources.map(s=>s.id);
    const regionalIds=region?await getRegionalArticleIds(region):null;
    if(region&&!regionalIds?.length)return[];
    let articleQuery=supabase.from("articles").select("id,source_id").eq("status","published").in("source_id",ids).not("image_url","is",null).neq("image_url","").limit(5000);
    if(regionalIds)articleQuery=articleQuery.in("id",regionalIds);
    ...
    return sources
      .map(source=>({id:source.id,name:source.name,slug:source.slug,websiteUrl:source.website_url,logoUrl:source.logo_url,languageCode:source.default_language_code||"en",articleCount:counts.get(source.id)||0,lastSuccessAt:source.last_success_at}))
      .filter(source=>!region||source.articleCount>0);
  }catch(error){console.error("getSourceDirectory",error);return[];}
}

export async function getMostReadStories(limit=30, region?:EditionRegion):Promise<PortalStory[]>{
  try{
    const supabase=await createClient();
    const regionalIds=region?await getRegionalArticleIds(region):null;
    if(region&&!regionalIds?.length)return[];
    const metrics=await supabase.from("article_metrics").select("article_id,view_count").order("view_count",{ascending:false}).limit(Math.min(Math.max(limit*8,100),1000));
    if(metrics.error||!metrics.data?.length)return[];
    const rankedMetrics=regionalIds?metrics.data.filter(row=>regionalIds.includes(row.article_id)):metrics.data;
    const selectedMetrics=rankedMetrics.slice(0,limit);
    if(!selectedMetrics.length)return[];
    const ids=selectedMetrics.map(r=>r.article_id);
    const articles=await supabase.from("articles").select("id,source_id,slug,title,description,ai_summary,image_url,language_code,published_at,discovered_at").in("id",ids).eq("status","published").not("image_url","is",null).neq("image_url","");
    if(articles.error)throw articles.error;
    const rowById=new Map((articles.data??[]).map(r=>[r.id,r]));
    const ordered=ids.map(id=>rowById.get(id)).filter(Boolean) as any[];
    const counts=new Map(selectedMetrics.map(r=>[r.article_id,Number(r.view_count||0)]));
...
export async function getDistrictGroups():Promise<DistrictGroup[]>{
  const stories=await getPortalStories({region:"sri-lanka",limit:200});
  const groups:DistrictGroup[]=DISTRICTS.map(name=>({name,slug:slugify(name),count:0,stories:[]}));
  for(const story of stories){const haystack=`${story.title} ${story.summary}`.normalize("NFKC").toLocaleLowerCase();for(const group of groups){if(DISTRICT_ALIASES[group.name].some(alias=>haystack.includes(alias.toLocaleLowerCase()))){group.stories.push(story);group.count+=1;}}}
