...
export default async function SourcePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!source) notFound();
  const stories = await getPortalStories({ region, sourceSlug: slug, limit: 80 });
  ...
  return (
    <main className="mx-auto max-w-6xl px-5 py-10">
      <div className="flex flex-col gap-5 border-b border-gray-200 pb-7 sm:flex-row sm:items-center">
        ...
        <div className="min-w-0 flex-1"><p className="text-[11px] font-black uppercase tracking-[0.18em] text-gray-400">{edition.flag} {edition.label} · Publisher</p><h1 className="mt-1 text-4xl font-black tracking-tight">{source.name}</h1><p className="mt-2 text-sm text-gray-500">{source.languageCode.toUpperCase()} · {stories.length} recent {edition.label} stor{stories.length === 1 ? "y" : "ies"}</p></div>
