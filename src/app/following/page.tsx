...
export default async function FollowingPage() {
  ...
  const [sourceFollows, topicFollows, preferences, regionalSources, regionalTopics] = await Promise.all([
    ...
    getSourceDirectory(200, region),
    getTopics(200, region),
  ]);

  const allowedSourceIds = new Set(regionalSources.map((source) => source.id));
  const visibleSourceFollows = (sourceFollows.data ?? []).filter((follow) => allowedSourceIds.has(follow.source_id));
  const sourceIds = visibleSourceFollows.map((row) => row.source_id);
  ...
  const allowedTopicKeys = new Set(regionalTopics.map((topic) => topic.slug));
  const visibleTopicFollows = (topicFollows.data ?? []).filter((topic) => allowedTopicKeys.has(topic.topic_key));
  const prefs = preferences.data;

  return (
    <main className="mx-auto max-w-6xl px-5 py-10">
      <div className="border-b border-gray-200 pb-6">
        <p className="text-[11px] font-black uppercase tracking-[0.18em] text-gray-400">{edition.flag} {edition.label} · Personal feed</p>
        <h1 className="mt-2 text-4xl font-black tracking-tight md:text-5xl">Following in {edition.label}</h1>
        <p className="mt-3 max-w-3xl leading-7 text-gray-600">Only followed publishers and topics that currently belong to your selected edition are shown. Switch editions to manage follows from another region.</p>
      </div>

      <section className="mt-8 grid gap-6 lg:grid-cols-2">
        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between border-b border-gray-100 pb-4"><h2 className="text-2xl font-black">Publishers</h2><Link href="/sources" className="text-xs font-black text-gray-500">Find {edition.label} sources →</Link></div>
          <div className="divide-y divide-gray-100">
            {visibleSourceFollows.map((follow) => {
            ...
            {!visibleSourceFollows.length ? <p className="py-6 text-sm text-gray-500">You are not following any publishers in the {edition.label} edition.</p> : null}
          </div>
        </div>

        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between border-b border-gray-100 pb-4"><h2 className="text-2xl font-black">Topics</h2><Link href="/topics" className="text-xs font-black text-gray-500">Browse {edition.label} topics →</Link></div>
          <div className="divide-y divide-gray-100">
            {visibleTopicFollows.map((topic) => <div key={topic.topic_key} className="flex items-center gap-3 py-4"><div className="grid h-11 w-11 place-items-center rounded-xl bg-gray-950 text-white">#</div><div className="min-w-0 flex-1"><Link href={`/topics/${topic.topic_key}`} className="font-black hover:underline">{topic.topic_label}</Link><p className="mt-0.5 text-xs text-gray-400">{edition.label} topic follow</p></div><form action={unfollowTopic}><input type="hidden" name="topicKey" value={topic.topic_key} /><input type="hidden" name="returnTo" value="/following" /><button className="rounded-full border px-3 py-1.5 text-xs font-black hover:bg-gray-50">Unfollow</button></form></div>)}
            {!visibleTopicFollows.length ? <p className="py-6 text-sm text-gray-500">You are not following any active topics in the {edition.label} edition.</p> : null}
          </div>
        </div>
      </section>

      <section className="mt-6 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="max-w-3xl"><p className="text-[11px] font-black uppercase tracking-[0.18em] text-gray-400">Notification-ready preferences</p><h2 className="mt-1 text-2xl font-black">Choose what you want to hear about</h2><p className="mt-2 text-sm leading-6 text-gray-600">Notification settings stay with your account. Publisher and topic content shown above remains scoped to the selected edition.</p></div>
