...
export default async function Home() {
  ...
  const topStories = (sriLanka.length ? sriLanka : allLatest).slice(0, 6) as StoryWithImage[];
  ...
  return (
    <main className="mx-auto max-w-[1440px] px-3 py-4 sm:px-5 md:py-6">
      ...
      <section className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><StatWidget label="Recent stories" value={allLatest.length} sub="currently loaded" /><StatWidget label="Publishers" value={sourceCount} sub="different source feeds" /><StatWidget label="Topics" value={latestTopics.length} sub="grouped story topics" /><StatWidget label="Multi-source" value={multiSourceTopics} sub="cross-source coverage" /></section>
      ...
      <section className="mt-5 grid gap-5 xl:grid-cols-[1fr_330px]">
        <div className="rounded-xl border border-gray-200 bg-white p-4 md:p-5"><SectionTitle eyebrow="Live feed" title="Latest News" href="/latest" /><div>{feed.map((topic) => <TopicFeedRow key={topic.key} topic={topic} />)}</div></div>
        <aside className="space-y-4">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-4"><SectionTitle eyebrow="Browse" title="Quick Links" /><div className="mt-4 grid grid-cols-2 gap-2 text-sm font-bold"><Link href="/region/sri-lanka" className="rounded-lg bg-red-600 px-3 py-3 text-white">🇱🇰 Sri Lanka</Link><Link href="/top-stories" className="rounded-lg bg-gray-950 px-3 py-3 text-white">Top Stories</Link><Link href="/latest" className="rounded-lg bg-gray-100 px-3 py-3">Latest</Link><Link href="/sources" className="rounded-lg bg-gray-100 px-3 py-3">Sources</Link><Link href="/map" className="rounded-lg bg-gray-100 px-3 py-3">Map</Link><Link href="/official" className="rounded-lg bg-gray-100 px-3 py-3">Official</Link></div></div>
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-3">{[["🇱🇰", "Sri Lanka", "/region/sri-lanka", sriLanka],["🇦🇺", "Australia", "/region/australia", australia],["🌍", "International", "/region/international", international]].map(([flag, label, href, items]) => <div key={label as string} className="rounded-xl border border-gray-200 bg-white p-4"><div className="flex items-center justify-between border-b border-gray-200 pb-3"><h2 className="text-xl font-black">{flag as string} {label as string}</h2><Link href={href as string} className="text-xs font-black text-gray-500">View all →</Link></div><div className="mt-1">{(items as StoryWithImage[]).slice(0, 6).map((story) => <MiniHeadline key={story.slug} story={story} />)}</div></div>)}</section>
    </main>
  );
}
