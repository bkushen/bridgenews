...
function MiniHeadline({ story }: { story: StoryWithImage }) {
  return (
    <Link href={`/story/${story.slug}`} className="group flex gap-3 border-b border-gray-100 py-3 last:border-0">
      {story.imageUrl ? (
        <img src={story.imageUrl} alt="" className="h-16 w-20 shrink-0 rounded-xl object-cover" />
      ) : (
        <div className="h-16 w-20 shrink-0 rounded-xl bg-gradient-to-br from-gray-200 to-gray-100" />
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-[160px_1fr]">
        ...
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {topic.stories.slice(0, 4).map((story) => <span key={story.slug} className="rounded-full border border-gray-200 bg-gray-50 px-2.5 py-1 text-[10px] font-bold text-gray-600">{story.source}</span>)}
          </div>
        </div>
      </div>
    </article>
  ...
  return (
    <main className="mx-auto max-w-[1440px] px-3 py-4 sm:px-5 md:py-6">
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-[1fr_330px]">
        <div className="grid gap-3 lg:grid-cols-[1.55fr_0.85fr]">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
            {heroStories.slice(1, 3).map((story) => <HeroCard key={story.slug} story={story} />)}
          </div>
        </div>
      ...
      <section className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <StatWidget label="Published" value={allLatest.length} sub="recent stories loaded" />
        <StatWidget label="Publishers" value={sourceCount} sub="different source feeds" />
      ...
      <section className="mt-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <div className="shrink-0"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-gray-400">Trending topics</p><p className="text-sm font-black">Explore what is moving</p></div>
          <div className="flex flex-wrap gap-2 lg:border-l lg:border-gray-200 lg:pl-4">
            {topCategories.map((category) => <Link key={category.slug} href={`/categories/${category.slug}`} className="rounded-full border border-gray-200 bg-gray-50 px-3 py-2 text-xs font-black hover:border-gray-400 hover:bg-white">#{category.name} <span className="ml-1 text-gray-400">{category.articleCount}</span></Link>)}
          </div>
        </div>
      </section>
      ...
      <section className="mt-5 grid gap-5 xl:grid-cols-[1fr_330px]">
        <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm md:p-5">
          <SectionTitle eyebrow="Live feed" title="Latest News" href="/latest" />
          <div>{feed.map((topic) => <TopicFeedRow key={topic.key} topic={topic} />)}</div>
        </div>
        ...
        <aside className="space-y-4">
            ...
            <div className="mt-2 divide-y divide-gray-100">
              {trendingTopics.slice(0, 7).map((topic, index) => (
                <Link key={topic.key} href={`/story/${topic.lead.slug}`} className="flex gap-3 py-3">
                  <span className="text-xl font-black text-gray-300">{String(index + 1).padStart(2, "0")}</span>
                  <div><p className="line-clamp-2 text-sm font-black leading-5 hover:underline">{topic.lead.title}</p><p className="mt-1 text-[11px] font-semibold text-gray-500">{topic.sourceCount} sources · {topic.stories.length} reports</p></div>
                </Link>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-gray-950 p-5 text-white shadow-sm">
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-gray-400">BridgeNews Pulse</p>
            <h3 className="mt-2 text-2xl font-black">Sri Lanka first</h3>
            <p className="mt-2 text-sm leading-6 text-gray-300">Fast coverage from multiple Sri Lankan publishers, with English, Sinhala and Tamil sources grouped into one live portal.</p>
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full w-[78%] rounded-full bg-white" /></div>
            <div className="mt-2 flex justify-between text-[10px] font-bold uppercase tracking-wider text-gray-400"><span>Coverage</span><span>Growing</span></div>
          </div>

          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            <SectionTitle eyebrow="Browse" title="Quick Links" />
            <div className="mt-4 grid grid-cols-2 gap-2 text-sm font-bold">
              <Link href="/region/sri-lanka" className="rounded-xl bg-red-600 px-3 py-3 text-white">🇱🇰 Sri Lanka</Link>
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-3">
        {[
          ...
          <div key={label as string} className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-1">{(items as StoryWithImage[]).slice(0, 6).map((story) => <MiniHeadline key={story.slug} story={story} />)}</div>
          </div>
        ))}
      </section>
    </main>
