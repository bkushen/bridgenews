...
export default async function ArchivePage({ searchParams }: { searchParams: Promise<{ date?: string; language?: string }> }) {
  ...
  const date = /^\d{4}-\d{2}-\d{2}$/.test(params.date || "") ? params.date! : today(edition.timeZone);
  const language = ["en", "si", "ta"].includes(params.language || "") ? params.language! : "all";
  const stories = await getPortalStories({ region, date, language: language === "all" ? undefined : language, limit: 120 });
  ...
  return (
    <main className="mx-auto max-w-6xl px-5 py-10">
      <div className="flex flex-col gap-5 border-b border-gray-200 pb-6 lg:flex-row lg:items-end lg:justify-between">
        <div><p className="text-[11px] font-black uppercase tracking-[0.18em] text-gray-400">{edition.flag} {edition.label} · By date</p><h1 className="mt-2 text-4xl font-black tracking-tight md:text-5xl">{edition.label} News Archive</h1><p className="mt-3 text-gray-600">Browse {edition.adjective} BridgeNews coverage by publication date and language.</p></div>
      ...
      <section className="mt-6 rounded-2xl border border-gray-200 bg-white px-5 shadow-sm">{stories.length ? stories.map((story) => <PortalStoryRow key={story.articleId} story={story} />) : <p className="py-10 text-center text-gray-500">No published {edition.label} stories found for this date and language.</p>}</section>
    </main>
  );
}
