...
export default async function SavedPage() {
  ...
  if (bookmarkedIds.length) {
    ...
    const regionLinks = regionRow.data
      ? await supabase.from("article_regions").select("article_id").eq("region_id", regionRow.data.id).in("article_id", bookmarkedIds)
      : { data: [], error: null };
    if (regionLinks.error) throw regionLinks.error;
    const articleIds = (regionLinks.data ?? []).map((row) => row.article_id);
    ...
    if (articleIds.length) {
      ...
      const order = new Map(bookmarkedIds.map((id, index) => [id, index]));
  ...
  return (
    <main className="mx-auto max-w-7xl px-5 py-10">
      <div className="max-w-3xl">
        <p className="text-[11px] font-black uppercase tracking-[0.18em] text-gray-400">{edition.flag} {edition.label} · Your account</p>
        <h1 className="mt-2 text-4xl font-black tracking-tight md:text-5xl">Saved in {edition.label}</h1>
        <p className="mt-3 text-base leading-7 text-gray-600">Only saved stories belonging to your selected edition are shown. Switch editions to see your other bookmarks.</p>
      </div>
      {stories.length ? (
        ...
        <div className="mt-8 rounded-2xl border border-dashed border-gray-300 bg-white p-8 text-gray-600">You have no saved stories in the {edition.label} edition.</div>
      )}
    </main>
  );
}
