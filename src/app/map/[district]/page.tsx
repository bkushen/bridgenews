...
export default async function DistrictPage({ params }: { params: Promise<{ district: string }> }) {
  ...
  if (region !== "sri-lanka") redirect("/map");
  ...
  return (
    <main className="mx-auto max-w-6xl px-5 py-10">
      <div className="border-b border-gray-200 pb-6"><p className="text-[11px] font-black uppercase tracking-[0.18em] text-emerald-700">🇱🇰 Sri Lanka · District coverage</p><h1 className="mt-2 text-4xl font-black tracking-tight md:text-5xl">📍 {group.name}</h1><p className="mt-3 text-gray-600">Recent Sri Lanka stories whose publisher headline or feed snippet mentions {group.name}.</p></div>
