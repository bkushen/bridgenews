...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string; language?: string; source?: string; date?: string; category?: string; topic?: string }> }) {
  ...
  const topicNeedle = selectedTopic?.name.toLocaleLowerCase() || "";
  ...
  const stories = rawStories.filter((story) => {
    ...
    if (topic !== "all" && topicNeedle && !`${story.title} ${story.summary} ${story.category}`.toLocaleLowerCase().includes(topicNeedle)) return false;
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-5 sm:py-10">
      <div className="max-w-3xl border-b border-[var(--news-line)] pb-6">
        ...
        <p className="mt-3 max-w-2xl text-base leading-7 text-[var(--news-muted)]">Search is locked to your selected edition. Switch editions in the header to search a different region.</p>
      </div>
      ...
      <form action="/search" method="get" className="mt-6 rounded-xl border border-[var(--news-line)] bg-[var(--news-card)] p-4 sm:p-5">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <label className="md:col-span-2 xl:col-span-2"><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.14em] text-[var(--news-muted)]">Search {edition.label}</span><input name="q" defaultValue={query} placeholder="News, people, places or topics" className={`${controlClass} w-full`} /></label>
          <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.14em] text-[var(--news-muted)]">Language</span><select name="language" defaultValue={language} className={`${controlClass} w-full font-bold`}><option value="all">All languages</option><option value="en">English</option>{region === "sri-lanka" ? <><option value="si">සිංහල</option><option value="ta">தமிழ்</option></> : null}</select></label>
          ...
          <div className="flex items-end"><span className="w-full rounded-lg border border-[var(--news-line)] bg-[var(--news-paper)] px-3.5 py-3 text-sm font-bold text-[var(--news-muted)]">{edition.flag} {edition.label} edition</span></div>
        </div>
