  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-5 sm:py-10">
      <div className="max-w-3xl border-b border-[var(--news-line)] pb-6">
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--news-accent)]">Discovery</p>
        <h1 className="mt-2 text-4xl font-bold tracking-tight text-[var(--news-ink)] md:text-5xl">Search BridgeNews</h1>
        <p className="mt-3 max-w-2xl text-base leading-7 text-[var(--news-muted)]">Search published headlines and snippets, then narrow the results by region, language, publisher, category, topic or date.</p>
      </div>
      ...
      <form action="/search" method="get" className="mt-6 rounded-xl border border-[var(--news-line)] bg-[var(--news-card)] p-4 sm:p-5">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.14em] text-[var(--news-muted)]">Region</span><select name="region" defaultValue={region} className={`${controlClass} w-full font-bold`}><option value="all">All regions</option>{regions.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}</select></label>
      ...
      {query ? (
        <section className="mt-9">
          <div className="mb-5 border-b border-[var(--news-line)] pb-3"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[var(--news-accent)]">Results</p><h2 className="mt-1 text-2xl font-bold text-[var(--news-ink)]">{stories.length} result{stories.length === 1 ? "" : "s"} for “{query}”</h2></div>
          {stories.length ? <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{stories.map((story) => <StoryCard key={story.slug} story={story} />)}</div> : <div className="rounded-xl border border-dashed border-[var(--news-line)] bg-[var(--news-card)] p-8 text-[var(--news-muted)]"><p className="font-serif text-xl font-bold text-[var(--news-ink)]">No matching stories</p><p className="mt-2 text-sm leading-6">Try a broader keyword or remove one or more filters.</p></div>}
        </section>
      ) : <section className="mt-8 rounded-xl border border-[var(--news-line-soft)] bg-[#f4eee5] p-6"><p className="font-serif text-xl font-bold text-[var(--news-ink)]">Start with a headline, person or place</p><p className="mt-2 text-sm leading-6 text-[var(--news-muted)]">You can then refine the results using the filters above.</p></section>}
    </main>
  );
}
