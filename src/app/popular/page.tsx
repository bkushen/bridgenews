...
export default async function PopularPage() {
  ...
  const [mostRead, stories] = await Promise.all([
    getMostReadStories(30, region),
  ...
  return (
    <main className="mx-auto max-w-7xl px-5 py-10">
      <div className="border-b border-gray-200 pb-6"><p className="text-[11px] font-black uppercase tracking-[0.18em] text-gray-400">{edition.flag} {edition.label} · Audience & coverage</p><h1 className="mt-2 text-4xl font-black tracking-tight md:text-5xl">Popular in {edition.label}</h1><p className="mt-3 max-w-3xl leading-7 text-gray-600">Most Read measures BridgeNews reader activity inside this edition only. Most Covered measures how many independent {edition.adjective} publishers are reporting the same topic.</p></div>
