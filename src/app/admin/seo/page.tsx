...
export default async function SeoPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  return <main className="px-4 py-6 sm:px-6 lg:px-8 lg:py-8"><AdminFeedback saved={params.saved==="1"} error={params.error??null}/>
    ...
    <form action={saveSeoSettings} className="mt-6 max-w-3xl space-y-4 rounded-3xl border border-slate-200 bg-white p-5 sm:p-6">
      ...
      <button className="rounded-xl bg-black px-5 py-2.5 text-sm font-black text-white hover:bg-slate-800">Save SEO settings</button>
    </form>
  </main>;
}
