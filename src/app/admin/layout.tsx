        ...
        <aside className="admin-sidebar sticky top-0 hidden h-screen overflow-hidden bg-slate-950 text-white shadow-2xl lg:flex lg:flex-col">
          <div className="border-b border-white/8 px-5 py-5">
            <Link href="/admin" className="flex items-center gap-3">
              ...
              <div><p className="font-black leading-tight text-white">BridgeNews</p><p className="mt-0.5 text-xs font-semibold text-slate-400">Admin control center</p></div>
            </Link>
          </div>
          ...
          <nav className="flex-1 overflow-y-auto px-3 py-5">
            {GROUPS.map((group) => (
              <div key={group.label} className="mb-6">
                <p className="px-3 pb-2 text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">{group.label}</p>
          ...
          <div className="border-t border-white/8 p-3">
            <div className="mb-2 rounded-2xl bg-white/5 p-3 ring-1 ring-white/8">
            ...
            <form action={logout} className="mt-1">
              <button className="w-full rounded-xl px-3 py-2.5 text-left text-sm font-bold text-rose-300 hover:bg-rose-500/10 hover:text-rose-200">Sign out</button>
            </form>
          </div>
        </aside>
