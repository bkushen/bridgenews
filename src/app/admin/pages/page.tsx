...
export default async function AdminPages({ searchParams }: { searchParams: Promise<{ saved?: string; deleted?: string; error?: string }> }) {
  ...
  return <main className="px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
    ...
    <details className="mt-6 rounded-2xl border border-slate-200 bg-white p-4"><summary className="cursor-pointer font-black text-slate-900">+ Create a new page</summary><PageForm /></details>

    <div className="mt-5 space-y-4">{pages.map((page:any)=><article key={page.id} className="rounded-2xl border border-slate-200 bg-white p-5">
...
function PageForm({ page }: { page?: any }) {
  return <form action={saveContentPage} className="mt-4 grid gap-4 lg:grid-cols-[1fr_320px]">
    ...
    <aside className="space-y-3 rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200"><Field label="SEO title"><input name="seo_title" defaultValue={page?.seo_title ?? ""} className={inputClass}/></Field><Field label="SEO description"><textarea name="seo_description" rows={4} defaultValue={page?.seo_description ?? ""} className={inputClass}/></Field><label className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm font-bold"><input type="checkbox" name="published" defaultChecked={page?.published ?? true}/>Published</label><button className="w-full rounded-xl bg-black px-4 py-2.5 text-sm font-black text-white hover:bg-slate-800">Save page</button>{page?<ConfirmSubmitButton formAction={deleteContentPage} label="Delete page" confirmMessage={`Delete ${page.title}?`} className="w-full rounded-xl border border-rose-200 bg-white px-4 py-2.5 text-sm font-black text-rose-600 hover:bg-rose-50"/>:null}</aside>
  </form>;
}
