...
const QUICK_ACTIONS = [
  ...
  ["Settings", "Branding and defaults", "/admin/settings", "settings"],
] as const;
...
export default async function AdminPage() {
  ...
  return <main className="px-4 py-6 sm:px-6 lg:px-8 lg:py-8 xl:px-10">
    ...
    <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-5 py-4"><div><p className="text-sm font-black text-slate-900">System safeguards</p><p className="mt-0.5 text-xs text-slate-500">AI processing is off. Repeatedly failing sources auto-pause after 12 failures.</p></div><Link href="/admin/audit" className="text-xs font-black text-slate-500 hover:text-black">{audit.toLocaleString()} admin events →</Link></div>
  </main>;
}
