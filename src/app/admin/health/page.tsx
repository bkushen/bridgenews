...
import { explainSourceError } from "@/lib/admin/source-health";
...
export default async function HealthPage() {
  ...
  return <main className="px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
    ...
    <div className="mt-6 grid gap-5 xl:grid-cols-2"><Panel title="Sources needing attention">{broken.length?broken.slice(0,20).map((s:any)=>{const e=explainSourceError(s.last_error_message,s.enabled);return <div key={s.id} className="border-b border-slate-100 py-3 last:border-0"><div className="flex items-center justify-between gap-3"><p className="font-black">{s.name}</p><span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${s.enabled?"bg-amber-50 text-amber-700":"bg-slate-100 text-slate-500"}`}>{s.enabled?"Enabled":"Disabled"}</span></div><p className="mt-1 text-xs font-bold text-slate-600">{e.title}</p><p className="mt-1 text-xs leading-5 text-slate-500">{e.action}</p></div>}):<Empty text="No source errors recorded."/>}</Panel>
