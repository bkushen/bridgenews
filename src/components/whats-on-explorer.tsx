...
const CATEGORIES: Array<{ key: "all" | WhatsOnCategory; label: string; icon: string }> = [
  ...
  { key: "theatre", label: "Stage Dramas", icon: "🎭" },
  ...
  return <section className={`rounded-xl border border-[#ded5c8] bg-[#fffdf8] ${compact ? "p-4" : "p-5 sm:p-7"}`}>
    ...
    {!selected ? <div className="mt-5 rounded-xl border border-dashed border-[#d9cfc2] bg-[#f8f3eb] p-6 text-center"><p className="font-serif text-lg font-bold text-[#1d1b19]">No pictured listings are available right now.</p><p className="mt-2 text-xs leading-5 text-[#746a61]">BridgeNews only shows genuine movie/event posters here. Listings without a usable image stay out of this visual module.</p></div> : <>
      <div className={`mt-4 ${compact ? "grid grid-cols-[145px_minmax(0,1fr)] gap-4" : "grid gap-5 rounded-xl border border-[#ddd3c6] bg-[#fffdf8] p-4 md:grid-cols-[260px_minmax(0,1fr)]"}`}>
        <button type="button" onClick={() => window.open(selected.ticketUrl, "_blank")} className="group overflow-hidden rounded-lg bg-[#eee5da] text-left shadow-sm">
          <img src={selected.imageUrl!} alt={`${selected.title} poster`} className={`${compact ? "h-[218px]" : "h-[390px]"} w-full object-cover transition duration-300 group-hover:scale-[1.025]`} />
        </button>
        <div className="min-w-0 self-start">
          <div className="flex flex-wrap gap-1.5 text-[9px] font-black uppercase tracking-wide">
            <span className="rounded border border-[#e1aaa9] bg-[#faeeee] px-2 py-1 text-[#8b1e28]">{LABELS[selected.category]}</span>
            {selected.language ? <span className="rounded border border-[#d8d0c4] px-2 py-1 text-[#60584f]">{selected.language}</span> : null}
            {selected.duration ? <span className="rounded border border-[#d8d0c4] px-2 py-1 text-[#60584f]">⏱ {selected.duration}</span> : null}
          ...
          <div className="mt-3 space-y-1 text-[11px] text-[#746a61]">
            {selected.status ? <p>🎟 {selected.status}</p> : null}
            <p>📍 {[selected.venue, selected.city].filter(Boolean).join(", ") || regionLabel}</p>
          </div>
          <div className="mt-3 flex flex-wrap gap-1.5">
            {selected.providers.slice(0, compact ? 5 : 10).map((provider) => <a key={`${provider.name}-${provider.url}`} href={provider.url} target="_blank" rel="noreferrer" className="rounded border border-[#a5232f] px-2 py-1 text-[9px] font-black uppercase text-[#a5232f] transition hover:bg-[#a5232f] hover:text-white">{provider.name} ↗</a>)}
          </div>
        </div>
      </div>
