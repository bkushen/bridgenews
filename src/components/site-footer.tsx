...
export async function SiteFooter({ siteName = "BridgeNews" }: { siteName?: string }) {
  ...
  return <footer data-public-footer className="mt-16 border-t">
    <div className="mx-auto grid max-w-[1180px] gap-10 px-5 py-12 md:grid-cols-[1.35fr_1fr]">
      ...
      <div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#9f9488]">Explore</p><div className="mt-4 grid grid-cols-2 gap-3 text-sm font-semibold text-[#ddd4c9] sm:grid-cols-3">{menu.map((item:any)=><Link key={item.id} href={item.href} className="news-footer-link transition">{item.label}</Link>)}</div></div>
    </div>
