  ...
  return (
    <section className="mt-4 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
      <div className="flex items-center justify-between gap-3 border-b border-gray-200 px-4 py-3">
        <div className="flex items-center gap-3"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-gray-400">Publishers</p><p className="text-sm font-black">Live sources</p></div>
        <Link href="/sources" className="text-[10px] font-black text-gray-500 hover:text-black">View directory →</Link>
      </div>
      <div className="source-rail-mask overflow-hidden py-4">
        <div className="source-rail-track flex w-max gap-3 px-4">{loop.map((source, index) => <SourceCard key={`${source.id}-${index}`} source={source} />)}</div>
      </div>
    </section>
  );
}
